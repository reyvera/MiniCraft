...
MiniCraft.resetWorld = function() {
  ...
  st.playerBuilt = Array.from(
    { length: MiniCraft.CONFIG.WORLD_H },
    () => Array.from({ length: MiniCraft.CONFIG.WORLD_W }, () => 0)
  );
...
MiniCraft.update = function() {
  ...
  const up    = st.keys.has("arrowup");
  const down  = st.keys.has("arrowdown");
  const left  = st.keys.has("arrowleft");
  const right = st.keys.has("arrowright");
